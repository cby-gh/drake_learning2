...
void WorldPointEvaluator<T>::EvalFullJacobian(
  ...
  //Eigen::Matrix<double, 3, Dynamic> test;
//	drake::Matrix3X<T> test;
//test << 0,
//     0, 
//     0;	
  plant().CalcJacobianTranslationalVelocity(
	...
	//std::cout<<"JJJJJJJ1111111:\n "<<*J<<std::endl;   
  *J = rotation_ * (*J);
	//std::cout<<"JJJJJJJ22222222:\n "<<*J<<std::endl;   
}
...
WorldPointEvaluator<T>::CreateLinearFrictionConstraints(int num_faces) const {
  ...
  if (is_frictional_) {
    ...
    Vector3d ub = Vector3d::Constant(std::numeric_limits<double>::infinity());
    lb(2) = 0;
    constraints.push_back(std::make_shared<BoundingBoxConstraint>(lb, ub));//lb<=x<=ub
  }
  return constraints;
}
