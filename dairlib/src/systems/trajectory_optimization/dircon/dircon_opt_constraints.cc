    ...
    : NonlinearConstraint<T>(
          ...
          1 +
              2 * (plant.num_positions() + plant.num_velocities() +
                   plant.num_actuators()) +
              //(4 * evaluators.get_evaluator(0).num_full()) +
      ...
      //n_l_(evaluators.get_evaluator(0).num_full()),
      cache_(cache) {
          //if(mode_index==0)
          //std::cout<<"NonlinearConstraint<T>::constraint.get_description(): "<<NonlinearConstraint<T>::get_description()<<std::endl;
      }
...
void DirconCollocationConstraint<T>::EvaluateConstraint(
  ...
  const auto& quat_slack =
   ...
   //  std::unique_ptr<Context<T>>  contextnow = plant.CreateDefaultContext();
   //std::cout <<" plant->get_actuation_input_port().get_index()"<<plant_.get_actuation_input_port().get_index()<<std::endl;  
    //std::cout <<" plant->get_actuation_input_port().GetFullDescription()"<<plant_.get_actuation_input_port().GetFullDescription()<<std::endl;     
    //    std::cout <<" plant->get_actuation_input_port().HasValue()"<<plant_.get_actuation_input_port().HasValue(* context_0_)<<std::endl;     
    //        std::cout <<" plant->get_actuation_input_port().HasValue()"<<plant_.get_actuation_input_port().HasValue(* context_1_)<<std::endl;    
  ...
  //std::cout<<"xcolxcolxcolxcol:\n "<<xcol.segment(8, 8)<<std::endl;    
  //std::cout<<"xdotcolxdotcolxdotcolxdotcol:\n "<<xdotcol.segment(0, 8)<<std::endl;    
  static drake::MatrixX<T> J(evaluators_.count_full(), plant_.num_velocities());
  //static drake::MatrixX<T> J(evaluators_.get_evaluator(0).num_full(), plant_.num_velocities());
...
//std::cout<<"quat_start_indices_.size() "<<quat_start_indices_.size()<<std::endl; 
...
drake::VectorX<T> DirconCollocationConstraint<T>::CalcTimeDerivativesWithForce(
  ...
  if (cache_) {
     // std::cout<<"cache_ is not empty222" <<std::endl;
      auto xxcxvc =   evaluators_.CalcMassMatrixTimesVDot(*context, forces);
    return cache_->CalcTimeDerivativesWithForce(context, forces);
      
  } else {
     // std::cout<<"cache_ is  empty111" <<std::endl;
    auto xxcxvc =   evaluators_.CalcMassMatrixTimesVDot(*context, forces);
...
ImpactConstraint<T>::ImpactConstraint(
    ...
    : NonlinearConstraint<T>(
          ...
          plant.num_positions() + 2 * plant.num_velocities() +//evaluators.get_evaluator(0).num_full(),
...
void ImpactConstraint<T>::EvaluateConstraint(
  ...
  *y = M * (v1 - x0.tail(plant_.num_velocities()))-evaluators_.EvalFullJacobian(*context_).transpose() * impulse;//动量观测器?
}
...
CachedAccelerationConstraint<T>::CachedAccelerationConstraint(
    ...
    : NonlinearConstraint<T>(
          ...
          plant.num_positions() + plant.num_velocities() +
              ...
              //plant.num_actuators() + evaluators.get_evaluator(0).num_full(),
...
void CachedAccelerationConstraint<T>::EvaluateConstraint(
 ...
 //const auto& lambda = vars.tail(evaluators_.get_evaluator(0).num_full());
  ...
  if (cache_) {
    ...
    *y = evaluators_.EvalActiveSecondTimeDerivative(context_, lambda);
      //std::cout<<"out of  cache"<<std::endl;
  }
}
