...
#include "extensionx_trajectory/extension11_trajectory.h"
#include "drake/systems/rendering/multibody_position_to_geometry_pose.h"
DEFINE_double(strideLength, 0.1, "The stride length.");
DEFINE_double(duration, 2.0, "The stride duration");//total time
...
void runDircon(
 ...
 //double min_T = .1;//The minimum total duration of the mode (default 0)
  ...
  double  min_T_mode2 = 0.5;//在这里直接修改左右相时间相等或者在类里添加时间相等约束
  double max_T_mode2 = 0.5;//
...
// planar_z = 1
 ...
 // trajopt.AddLinearConstraint(x0(positions_map["joint_left_ankle"]) ==0.0);
  //trajopt.AddLinearConstraint(x0(positions_map["joint_right_ankle"]) ==0.0);
  ...
  //trajopt.AddLinearConstraint(x0(nq + velocities_map["joint_left_ankledot"]) ==0.0);
  //trajopt.AddLinearConstraint(x0(nq + velocities_map["joint_right_ankledot"]) ==0.0);
...
//std::cout << "222" << std::endl;//finding_path2 <<std::endl;//drake/examples/PlanarWalker/PlanarWalker2.urdf
  trajopt.CreateVisualizationCallback(
      drake::FindResourceOrThrow("drake/examples/v10/v10_drake_extension4_112.urdf"),//e
  ...
  std::cout<<"trajopt.decision_variable(i): "<<trajopt.decision_variable(881)<<std::endl;//rel_offset_vars[0][0]=rel_x at mode0 
  std::cout<<"result.GetSolution(trajopt.decision_variable(881)): "<<result.GetSolution(trajopt.decision_variable(881))<<std::endl;
   std::cout<<"trajopt.decision_variable(i): "<<trajopt.decision_variable(1064)<<std::endl;//rel_offset_vars[1][0]=rel_x at mode1 
  std::cout<<"result.GetSolution(trajopt.decision_variable(1064)): "<<result.GetSolution(trajopt.decision_variable(1064))<<std::endl;
  std::cout<<"trajopt.decision_variable(i): "<<trajopt.decision_variable(1160)<<std::endl;//rel_offset_vars[2][0]=rel_x at mode2 
  std::cout<<"result.GetSolution(trajopt.decision_variable(1160)): "<<result.GetSolution(trajopt.decision_variable(1160))<<std::endl;
...
for(int i = 0; i<builder.GetMutableSystems().size();i++)
{
 ...
 auto test2 = diagram->CreateDefaultContext();
  std::cout<<"test2.to_string0:  "<<(*test2).to_string()<<std::endl;//8 x z rh lh rk lk ra la 
...
int main(int argc, char* argv[]) 
{
    ...
    std::srand(time(0));  
    auto plant = std::make_unique<drake::multibody::MultibodyPlant<double>>(0.0);//0.01.0E-4
    auto plant_vis = std::make_unique<MultibodyPlant<double>>(0.0);//0.01.0E-4
    ...
    std::string full_name = drake::FindResourceOrThrow("drake/examples/v10/v10_drake_extension4_112.urdf");//
    drake::FindResourceResult ResourceResult = drake::FindResource("drake/examples/v10/v10_drake_extension4_112.urdf");//
   ...
   int result1 = access("/home/cby/drake_learning/src/drake_learning2/src/x_planed_outsearch_select0.txtxxx", 0);//加入先验条件时,需要撤去文件名称的xxx
   int result2 = access("/home/cby/drake_learning/src/drake_learning2/src/u_planed_outsearch_select0.txtxxx", 0);
  if(result1==0&&result2==0)
  {
      std::cout<<"exist prior files xu"<<std::endl;
      std::fstream in_x_txt_file;
      std::fstream in_u_txt_file;
      in_x_txt_file.open("/home/cby/drake_learning/src/drake_learning2/src/x_planed_outsearch_select0.txt", std::ios::out | std::ios::in);
      in_u_txt_file.open("/home/cby/drake_learning/src/drake_learning2/src/u_planed_outsearch_select0.txt", std::ios::out | std::ios::in);
