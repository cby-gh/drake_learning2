...
#include "extensionx_trajectory/extension11_trajectory_fixed_ankle_pitch.h"

#include "drake/systems/rendering/multibody_position_to_geometry_pose.h"
DEFINE_double(strideLength, 0.1, "The stride length.");
DEFINE_double(duration, 2, "The stride duration");//total time
...
void runDircon(
    ...
    vector<PiecewisePolynomial<double>> init_vc_traj) 
 {
    ...
    std::cout<<"positions.rows()"<<positions.rows()<<std::endl;//7 x z pitch rh lh rk lk  
    std::cout<<"positions.cols()"<<positions.cols()<<std::endl;
    std::cout<<"positions.size()"<<positions.size()<<std::endl;
    std::cout<<"velocities.rows()"<<velocities.rows()<<std::endl;//7 x z rh lh rk lk 
    ...
    std::cout<<"context.to_string0:"<<(*context).to_string()<<std::endl;
    drake::VectorX<T> test(14);
...
//                                                             left_lower_leg, Matrix3d::Identity(), Vector3d::Zero(), {0, 2});//0 1 2对应所有xyz方向， 激活方向0 2 = x z
 ...
 //double min_T = .1;//The minimum total duration of the mode (default 0)
  ...
  double  min_T_mode0 = 0.25*2;//在这里直接修改左右相时间相等或者在类里添加时间相等约束
  double max_T_mode0 = 0.25*2;//
  double  min_T_mode1 = 0.5*2;//在这里直接修改左右相时间相等或者在类里添加时间相等约束
  double max_T_mode1 = 0.5*2;//
  double  min_T_mode2 = 0.25*2;//在这里直接修改左右相时间相等或者在类里添加时间相等约束
  double max_T_mode2 = 0.25*2;//
...
// planar_z = 1
  ...
  trajopt.AddLinearConstraint(trajopt.state_vars(1,9)[7] ==x0(nq + velocities_map["planar_xdot"]));//中点速度==起始速度
 // trajopt.AddLinearConstraint(xf(nq + velocities_map["planar_xdot"]) ==0);
  ...
  trajopt.AddLinearConstraint(x0(nq + velocities_map["joint_right_hipdot"])  ==  trajopt.state_vars(1,9)[10]);//蓝腿的髋关节速度==红腿的髋关节中间速度
  trajopt.AddLinearConstraint(x0(nq + velocities_map["joint_right_kneedot"])  == trajopt.state_vars(1,9)[12] );//蓝腿的膝关节速度==红腿的膝关节中间速度
...
/**/
 ...
 // trajopt.AddLinearConstraint(xf(positions_map["joint_right_ankle"]) ==x0(positions_map["joint_left_ankle"]) );
  ...
  trajopt.AddConstraintToAllKnotPoints(x(positions_map["planar_roty"])  == 0);  
...
//std::cout << "222" << std::endl;//finding_path2 <<std::endl;//drake/examples/PlanarWalker/PlanarWalker2.urdf
  trajopt.CreateVisualizationCallback(
      drake::FindResourceOrThrow("drake/examples/v10/v10_drake_extension4_112_fixed_ankle_pitch.urdf"),//e
  ...
  std::cout<<"trajopt.decision_variable(i): "<<trajopt.decision_variable(805)<<std::endl;//rel_offset_vars[0][0]=rel_x at mode0 when ankle is fixed
  std::cout<<"result.GetSolution(trajopt.decision_variable(805)): "<<result.GetSolution(trajopt.decision_variable(805))<<std::endl;
   std::cout<<"trajopt.decision_variable(i): "<<trajopt.decision_variable(987)<<std::endl;//rel_offset_vars[1][0]=rel_x at mode1 when ankle is fixed
  std::cout<<"result.GetSolution(trajopt.decision_variable(987)): "<<result.GetSolution(trajopt.decision_variable(987))<<std::endl;
  ...
  std::cout<<"result.GetSolution(trajopt.decision_variable(1082)): "<<result.GetSolution(trajopt.decision_variable(1082))<<std::endl;
  ...
  if(lammda_c_value_mode1==force_c_value_mode1)
   ...
   int gamma_c_index1 = 923;
    ...
    int lammda_index2 = 991;
    for(int i=0;i<10;i++)
    {
      for(int j=0;j<3;j++)
      {
        lammda_value_mode2(j,i) = decision_variables_value[lammda_index2];
        out_lammda_data_file2 << std::setprecision(4) << lammda_value_mode2(j,i)<< std::endl;//四位有效数字
        lammda_index2++;
  ...
  std::cout<<"no lammda_value_mode2!=force_value_mode2"<<std::endl;//579个值lammda0   
   ...
   int lammda_c_index2 = 1021;
    for(int i=0;i<9;i++)
    {
      for(int j=0;j<3;j++)
      {
        lammda_c_value_mode2(j,i) = decision_variables_value[lammda_c_index2];
        out_lammda_c_data_file2 << std::setprecision(4) << lammda_c_value_mode2(j,i)<<std::endl;//四位有效数字
        lammda_c_index2++;
      }
    }
   lammda_c_index2=lammda_c_index2-3;
    for(int j=0;j<3;j++)
    {
      out_lammda_c_data_file2 << std::setprecision(4) << decision_variables_value[lammda_c_index2]<<std::endl;//四位有效数字
      lammda_c_index2++;
    }
  ...
  std::cout<<"no lammda_c_value_mode2!=force_c_value_mode2"<<std::endl;//579个值lammda0   
    ...
    int gamma_c_index2 = 1048;
    for(int i=0;i<9;i++)
    {
      for(int j=0;j<3;j++)
      {
        gamma_c_value_mode2(j,i) = decision_variables_value[gamma_c_index2];
        out_gamma_c_data_file2 << std::setprecision(4) << gamma_c_value_mode2(j,i)<< std::endl;//四位有效数字
        gamma_c_index2++;
      }
    }
    ...
    for(int j=0;j<3;j++)
    {
      out_gamma_c_data_file2 << std::setprecision(4) << decision_variables_value[gamma_c_index2]<<std::endl;//四位有效数字
      gamma_c_index2++;
    }
...
int main(int argc, char* argv[]) 
{
    ...
    std::string full_name = drake::FindResourceOrThrow("drake/examples/v10/v10_drake_extension4_112_fixed_ankle_pitch.urdf");//
    drake::FindResourceResult ResourceResult = drake::FindResource("drake/examples/v10/v10_drake_extension4_112_fixed_ankle_pitch.urdf");//
    ...
    //frames 24(1world+13body frame+12 joint frame)
    ...
    out_ramdom_x_file.open("/home/cby/drake_learning/src/drake_learning2/src/searchdata/random_x_search112.txt", std::ios::out | std::ios::trunc);
    out_ramdom_u_file.open("/home/cby/drake_learning/src/drake_learning2/src/searchdata/random_u_search112.txt", std::ios::out | std::ios::trunc);
    int rand_out_result1 = access("/home/cby/drake_learning/src/drake_learning2/src/searchdata/random_x_search112.txt", 0);//启用文件操作时,需要撤去文件名称的xxx
    int rand_out_result2 = access("/home/cby/drake_learning/src/drake_learning2/src/searchdata/random_u_search112.txt", 0);
    if(rand_out_result1!=0||rand_out_result1!=0)
    {
      std::cout<<"no output rand file."<<std::endl;
      exit(100);
    }
    ...
    {
        init_time.push_back(i*0.0541);//timesteps
   ...
   int result1 = access("/home/cby/drake_learning/src/drake_learning2/src/priordata/u_planed_outsearch.txtxxx", 0);//加入先验条件时,需要撤去文件名称的xxx
   int result2 = access("/home/cby/drake_learning/src/drake_learning2/src/priordata/x_planed_outsearch.txtxxx", 0);
  if(result1==0&&result2==0)
  {
      std::cout<<"exist prior files xu"<<std::endl;
      std::fstream in_x_txt_file;
      std::fstream in_u_txt_file;
      in_x_txt_file.open("/home/cby/drake_learning/src/drake_learning2/src/priordata/u_planed_outsearch.txt", std::ios::out | std::ios::in);
      in_u_txt_file.open("/home/cby/drake_learning/src/drake_learning2/src/priordata/x_planed_outsearch.txt", std::ios::out | std::ios::in);
    ...
    for (int j = 0; j < 3; j++) //3 modes
    {
        ...
        else if(j==1)
        {
          ...
          if(result3==0&&result4==0&&result5==0)
          {
              ...
              in_lammda_data_file.open("/home/cby/drake_learning/src/drake_learning1/src/priordata/lammda_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::in);
              in_lammda_c_data_file.open("/home/cby/drake_learning/src/drake_learning1/src/priordata/lammda_c_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::in);
              in_gamma_c_data_file.open("/home/cby/drake_learning/src/drake_learning1/src/priordata/gammda_c_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::in);
